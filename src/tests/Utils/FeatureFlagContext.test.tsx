...
describe('FeatureFlags', (): void => {
	function renderWithProvider(ui: React.ReactElement, flags: FeatureFlags): void {
		const FeatureFlagsContext: Context<FeatureFlags> = React.createContext(flags);

		render(<FeatureFlagsContext.Provider value={flags}>{ui}</FeatureFlagsContext.Provider>);
	}
	...
	test('FeatureFlagsProvider provides default flags', (): void => {
		function TestComponent(): React.ReactElement {
			const flags: FeatureFlags = useFeatureFlags();
			return <div>Default flag: {flags.wishlistSettings ? 'true' : 'false'}</div>;
		}
		...
		const expected = process.env.REACT_APP_FEATURE_WISHLIST_SETTINGS === 'true' ? 'true' : 'false';

		expect(screen.getByText(`Default flag: ${expected}`)).toBeInTheDocument();
	});
});
